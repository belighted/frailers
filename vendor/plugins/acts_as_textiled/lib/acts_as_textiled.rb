    ...
    module Textiled
      ...
      module ClassMethods
        def acts_as_textiled(*attributes)
          ...
          type_options = %w( plain source with_code )
          ...
          attributes.each do |attribute|
            ...
            define_method("#{attribute}_with_code", proc  { strip_html(__send__(attribute)) if __send__(attribute) } )
      ...
      private
        ...
        def strip_html(html)
          html.dup.gsub(code_regexp) do |code|
            Uv.parse(strip_glyphs($2.strip), "xhtml", "ruby_on_rails", true, "blackboard")
          end
        end
        
        def strip_glyphs html
          returning html do |h|
